...
export namespace imageutil{
    ...
    export class ImageStream extends PropertyTarget{

        //region Static

        static fromFile(file: File, callback: (s: ImageStream) => void){
            ...
            reader.onload = (e) => {
                ...
                // Gets the data url
                let dataUrl = (<any>e.target).result;

                // Callback
                ImageStream.fromUrl(dataUrl, s => callback(s));
        ...
        static fromUrl(url: string, callback: (s: ImageStream) => void){
        ...
        static fromImage(image: HTMLImageElement): ImageStream{
        ...
        private resample_hermite(canvas: HTMLCanvasElement, W: number, H: number, W2: number, H2: number){
            let time1 = Date.now();
            W2 = Math.round(W2);
            H2 = Math.round(H2);
        ...
        crop(crop: ICropBounds, callback: (s: ImageStream) => void){

            this.toImage(null, image =>{
                crop.top = crop.top || 0;
        ...
        resize(options: ImageExportOptions, callback: (s: ImageStream) => void){
            ...
            this.toImage(null,image => {
                ...
                let target: Size = (fit == ImageFit.AspectFit ? original.scaleToFit(size) : original.scaleToFill(size)).round();
                ...
                this.resample_hermite(canvas, w, h, target.width, target.height);
        ...
        rotateCounterClockwise(callback: (s: ImageStream) => void){
            this.toImage(null,image => {
                let c = document.createElement('canvas');
        ...
        rotateClockwise(callback: (s: ImageStream) => void){
        ...
        toDataUrl(options: ImageExportOptions): string{
            let opts: ImageExportOptions = options || {} as any;
            return this.canvas.toDataURL(opts.type || ImageUtil.DEFAULT_TYPE, opts.quality || ImageUtil.DEFAULT_QUALITY);
        }

        toImage(options: ImageExportOptions, callback: (img: HTMLImageElement) => void){
    ...
    export class ImageUtil {
        ...
        private static resample_hermite(canvas: HTMLCanvasElement, W: number, H: number, W2: number, H2: number){
            let time1 = Date.now();
            W2 = Math.round(W2);
            H2 = Math.round(H2);
            let img = canvas.getContext("2d").getImageData(0, 0, W, H);
